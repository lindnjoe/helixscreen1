...
|-----|-------|-------------|
| `[.]` | 16 | Generic hidden (crashes, instability) |
...
**By Priority:**
- HIGH (blocks important testing): 11
- MEDIUM (reduces coverage): 17
...
### CommandSequencer Tests (2 tests)

| File | Line | Test | Tags | Issue |
|------|------|------|------|-------|
| `test_command_sequencer.cpp` | 87 | Queue management | `[sequencer][.]` | SIGILL crash during fixture destruction |
| `test_command_sequencer.cpp` | 139 | Start conditions | `[sequencer][.]` | Same fixture crash |

**Priority:** HIGH
**Root Cause:** Race between callback execution and fixture teardown
**Fix Approach:** Implement callback cancellation in SequencerTestFixture destructor

---

### Mock Print Simulation Tests (2 tests)

| File | Line | Test | Tags | Issue |
|------|------|------|------|-------|
| `test_mock_print_simulation.cpp` | 254 | Phase state machine | `[mock_print][phase][.]` | SIGSEGV during fixture destruction |
| `test_mock_print_simulation.cpp` | 396 | Speedup factor | `[mock_print][speedup][.]` | Same fixture crash |

**Priority:** HIGH
**Root Cause:** MockPrintTestFixture lifecycle - temperature thread not stopped
**Fix Approach:** Ensure all threads stopped and callbacks flushed before destruction
...
### Helix Macro Manager Tests (6 tests)

| File | Line | Test | Tags | Issue |
|------|------|------|------|-------|
| `test_helix_macro_manager.cpp` | 62 | Status check 1 | `[helix_macros][status][.]` | Fixture crash |
| `test_helix_macro_manager.cpp` | 69 | Status check 2 | `[helix_macros][status][.]` | Fixture crash |
| `test_helix_macro_manager.cpp` | 77 | Status check 3 | `[helix_macros][status][.]` | Fixture crash |
| `test_helix_macro_manager.cpp` | 85 | Status check 4 | `[helix_macros][status][.]` | Fixture crash |
| `test_helix_macro_manager.cpp` | 220 | Install test 1 | `[helix_macros][install][.]` | Fixture crash |
| `test_helix_macro_manager.cpp` | 237 | Install test 2 | `[helix_macros][install][.]` | Fixture crash |

**Priority:** MEDIUM
**Root Cause:** Macro manager fixture cleanup issues
**Fix Approach:** Review fixture destructor; ensure async operations cancelled
...
### Moonraker Client Robustness Tests (3 tests)
...
**Root Cause:** Race condition in cleanup; behavior change; performance test
**Fix Approach:** Atomic flags for cleanup sync; review expected behavior; run manually
...
1. **Fix fixture destruction crashes** (11 tests)
   - CommandSequencer, MockPrint, HelixMacroManager
   - Pattern: Callbacks executing during/after destructor
   - Solution: Implement callback cancellation, use atomic flags

2. **Extend MoonrakerClientMock** (7 tests)
