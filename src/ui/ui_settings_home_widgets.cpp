...
/**
 * @file ui_settings_home_widgets.cpp
 * @brief Implementation of HomeWidgetsOverlay
 *
...
static std::unique_ptr<HomeWidgetsOverlay> g_home_widgets_overlay;

HomeWidgetsOverlay& get_home_widgets_overlay() {
    if (!g_home_widgets_overlay) {
        g_home_widgets_overlay = std::make_unique<HomeWidgetsOverlay>();
        StaticPanelRegistry::instance().register_destroy("HomeWidgetsOverlay",
                                                         []() { g_home_widgets_overlay.reset(); });
    }
    return *g_home_widgets_overlay;
}
...
HomeWidgetsOverlay::HomeWidgetsOverlay() {
...
void HomeWidgetsOverlay::init_subjects() {
...
void HomeWidgetsOverlay::register_callbacks() {
...
lv_obj_t* HomeWidgetsOverlay::create(lv_obj_t* parent) {
    if (overlay_root_) {
    ...
    overlay_root_ = static_cast<lv_obj_t*>(lv_xml_create(parent, "home_widgets_overlay", nullptr));
    if (!overlay_root_) {
...
void HomeWidgetsOverlay::show(lv_obj_t* parent_screen) {
...
void HomeWidgetsOverlay::on_activate() {
...
void HomeWidgetsOverlay::on_deactivate() {
...
static void on_widget_toggle_changed(lv_event_t* e) {
    LVGL_SAFE_EVENT_CB_BEGIN("[HomeWidgetsOverlay] on_widget_toggle_changed");
    ...
    if (!get_home_widgets_overlay().handle_widget_toggled(index, checked)) {
...
bool HomeWidgetsOverlay::handle_widget_toggled(size_t index, bool enabled) {
    if (!widget_config_) {
        return false;
    }
...
void HomeWidgetsOverlay::populate_widget_list() {
...
void HomeWidgetsOverlay::on_drag_handle_event(lv_event_t* e) {
    LVGL_SAFE_EVENT_CB_BEGIN("[HomeWidgetsOverlay] on_drag_handle_event");
    ...
    if (!from_switch) {
        auto& overlay = get_home_widgets_overlay();
...
static void drag_anim_completed_cb(lv_anim_t* anim) {
    (void)anim;
    get_home_widgets_overlay().finalize_drag();
}

void HomeWidgetsOverlay::handle_drag_start(lv_obj_t* row) {
    if (!widget_list_ || !row) {
        return;
    }
...
void HomeWidgetsOverlay::drag_scroll_timer_cb(lv_timer_t* timer) {
    auto* self = static_cast<HomeWidgetsOverlay*>(lv_timer_get_user_data(timer));
...
void HomeWidgetsOverlay::update_drag_auto_scroll() {
...
void HomeWidgetsOverlay::handle_drag_move() {
...
void HomeWidgetsOverlay::handle_drag_end() {
    if (!drag_active_ || !drag_row_ || !widget_list_ || !drag_placeholder_) {
        reset_drag_state();
        return;
    }
...
void HomeWidgetsOverlay::finalize_drag() {
...
void HomeWidgetsOverlay::reset_drag_state() {
